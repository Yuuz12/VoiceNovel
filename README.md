...
4. （可选）在段落中插入情感标签，如「今天天气真好。[高兴]」→ 合成时自动带上对应情感
5. （可选）点击右侧 **LLM 提取角色** 补充角色性别/描述信息
6. （可选）点击右侧 **一键智能匹配音色** 让 LLM 为角色分配音色
7. 也可手动为每个角色选择音色、调整语速/音量、试听
8. 点击播放器 ► 按钮开始播放，自动按段连播，对话用角色音色、旁白用旁白音色
9. 点击任意段落可跳转播放；播放进度实时显示
...
所有 provider 支持在段落文本中用方括号插入情感标签，例如：
...
支持的情感关键词：高兴、开心、快乐、悲伤、伤心、愤怒、生气、害怕、恐惧、厌恶、反感、惊讶、震惊、平静、冷静、中性。
